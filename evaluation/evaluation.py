"""
Evaluation script for CM2SHACL.
This script evaluates the practical applicability of CM2SHACL to the TED RDF mapping projects (standardForms and eForms).
It performs two main tasks:
1. Evaluates SHACL shape generation quality from conceptual mappings (CMs).
2. Validates generated RDF data against the extracted SHACL shapes.
It produces two CSV reports summarizing the results.
"""
...
def shapes_generation_analysis(repo_root, config_file, version, rows=[]):
    ...
    for suite in package_paths:
        ...
        output_path = suite.replace("data", "evaluation") + "/cm_shacl.ttl"
...
def rdf_validation(data_root="data", eval_root="evaluation"):
    ...
    for domain in sources:
        ...
        for package_path in glob.glob(os.path.join(domain_data_path, "package_*")):
            ...
            shacl_path = os.path.join(eval_package_path, "cm_shacl.ttl")
            ...
            for rdf_path in rdf_files:
                ...
                report_path = os.path.join(report_folder, "validation_report.ttl")
...
def validation_report_analysis(eval_root="evaluation", output_csv="evaluation/cm2shacl_validation_report_analysis.csv"):
    rows = []
    ...
    for source in sources:
        for package_path in glob.glob(os.path.join(eval_root, source, "package_*")):
            ...
            report_files = glob.glob(os.path.join(package_path, "**/validation_report.ttl"), recursive=True)
...
if __name__ == "__main__":
    # Evaluation part 1
    rows = []
    rows = shapes_generation_analysis("data/standardForms", "config.ini", "Conceptual Mapping Version 1.0", rows)
    rows = shapes_generation_analysis("data/eforms", "config.ini", "eForms 1.0", rows)
    write_report(rows, "evaluation/cm2shacl_report_analysis.csv")
